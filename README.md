...
This app relies on ElasticSearch. It is required to install the Elasticsearch app using Yunohost on the same server as the Timebank.cc app as it connects using the localhost port 9200. You can verify if the ElasticSearch server is running by: curl -X GET "localhost:9200".
